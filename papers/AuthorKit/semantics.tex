...
low, they are not rich enough to capture more subtle differences in the word meaning;
...
The verb 'to run' is translated into Czech with the same verb \textit{be(et}
for both senses, so this concrete ambiguity is not relevant for
the translation between English and Czech.  
If translated for example from English or Czech into Russian, the disambiguation is needed: 
while the verb in (1) will can be translated straightforwardly as \textit{be\v{z}a\v{t}} - 'to run' into Russian,
...
and some of their linguistic properties as features in machine learning.
